...
resource "google_project_iam_binding" "service_admin_permissions" {
  project = var.project
  role    = "roles/run.admin"
  members = [local.cloudbuild_serviceaccount,
  local.django_serviceaccount]

}

resource "google_project_iam_binding" "service_cloudsql_permissions" {
  project = var.project
